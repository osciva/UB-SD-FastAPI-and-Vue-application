...
from backend import repository, models, schemas, utils
from backend.schemas import SystemAccount
...
async def get_current_user(settings: Annotated[utils.Settings, Depends(get_settings)], db: Session = Depends(get_db),
...
def create_user(data: schemas.AccountCreate, db: Session = Depends(get_db)):
    ...
    if db_account:
        ...
        return repository.create_account(db=db, account=account_data, password=data.password)
